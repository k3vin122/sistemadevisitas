    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <x-nav-dropdown title="Modulos" align="right" width="48">
                        @can('view-any', App\Models\User::class)
                        <x-dropdown-link href="{{ route('users.index') }}">
                        Users
                        </x-dropdown-link>
                        @endcan
                        @can('view-any', App\Models\Estado::class)
                        <x-dropdown-link href="{{ route('estados.index') }}">
                        Estados
                        </x-dropdown-link>
                        @endcan
                        @can('view-any', App\Models\Proveedor::class)
                        <x-dropdown-link href="{{ route('proveedors.index') }}">
                        Proveedors
                        </x-dropdown-link>
                        @endcan
                        @can('view-any', App\Models\Registro::class)
                        <x-dropdown-link href="{{ route('registros.index') }}">
                        Registros
                        </x-dropdown-link>
                        @endcan
                </x-nav-dropdown>
                ...
                <x-nav-dropdown title="Gestión de acceso" align="right" width="48">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')">
                ...
                @endcan
                @can('view-any', App\Models\Proveedor::class)
                <x-responsive-nav-link href="{{ route('proveedors.index') }}">
                Proveedors
                </x-responsive-nav-link>
                @endcan
                @can('view-any', App\Models\Registro::class)
                <x-responsive-nav-link href="{{ route('registros.index') }}">
                Registros
                </x-responsive-nav-link>
                @endcan
